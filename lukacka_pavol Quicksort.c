...
void quickSort(char dvroz[MAX_SKEN_ZAPIS][MAX_SKEN], int lava, int prava);
int vyskyt(int space_count, char *pole_char,char dvroz[MAX_SKEN_ZAPIS][MAX_SKEN],char dvrozvys[MAX_SKEN_ZAPIS][MAX_SKEN]);
int vyskyt_vypis(int space_count, char *pole_char, int pocitadlo,char dvroz[MAX_SKEN_ZAPIS][MAX_SKEN],char dvrozvys[MAX_SKEN_ZAPIS][MAX_SKEN]);
...
int main()
{
    ...
    char pole_char[MAX_SKEN_ZAPIS],data_citanie[MAX_SKEN_ZAPIS],nazsub1[261],nazsub2[261];
...
//-----------------------------------------------------------------------------------CASE 1-------------------------------------------------------------------------------------------
    case 1: /* VYTVORENIE VLASTNEHO TEXTOVEHO SUBORU AKO VSTUP */
    {
        ...
        TICK(t_quickSortMain);

        for(i=0; i<TIMES; i++)
        {
            quickSort(dvroz,0 ,space_count);
        }

        TOCK(t_quickSortMain);
        ...
        vyskyt_vypis(space_count,pole_char,pocitadlo,dvroz,dvrozvys);
...
//-----------------------------------------------------------------------------------CASE 3 -------------------------------------------------------------------------------------------
    case 3:  /* TESTOVANIE*/
    {
        ...
        vyskyt_vypis(space_count,u,pocitadlo,dvroz,dvrozvys);
...
/*-------------------------------------------ASCII SORTOVANIE - QUICKSORT -----------------------------------------------------*/

void quickSort(char dvroz[MAX_SKEN_ZAPIS][MAX_SKEN], int lava, int prava)
{
    int i, j;
    char pom[MAX_SKEN_ZAPIS];

    i = lava;
    j = prava;
    x = dvroz[(lava+prava)/2];

    do
    {
        while((strcmp(dvroz[i],x) < 0) && (i < prava)) //POROVNAVAJ VSEKY SLOVA - PRAVA STRANA
        {
            i++;
        }
        while((strcmp(dvroz[j],x) > 0) && (j > lava)) //POROVNAVAJ VSEKY SLOVA - LAVA STRANA
        {
            j--;
        }
        if(i <= j)
        {
            strcpy(pom, dvroz[i]);      //VÝMENA SLOV CEZ POM
    ...
    while(i <= j); //AZ KYM SA I > J

    if(lava < j)   //REKURZÍVNE VOLANIE -
    {
        quickSort(dvroz, lava, j);
    }
    if(i < prava)  //REKURZÍVNE VOLANIE -
    {
        quickSort(dvroz, i, prava);
    }
}
...
int vyskyt_vypis(int space_count, char *pole_char, int pocitadlo,char dvroz[MAX_SKEN_ZAPIS][MAX_SKEN],char dvrozvys[MAX_SKEN_ZAPIS][MAX_SKEN])
{
    ...
    prenos = vyskyt(space_count,pole_char,dvroz,dvrozvys);
